...
import { seedPeople, seedSessions } from "./seeds";
...
const seeds:{[K in keyof TableMap]:TableMap[K][]}={schedule_sessions:seedSessions,people:seedPeople};
...
export async function listRows<K extends keyof TableMap>(table:K):Promise<TableMap[K][]> {if(!supabase)return localRead(table);const order=table==="schedule_sessions"?"day_number":"name";const{data,error}=await supabase.from(table).select("*").order(order);if(error)throw error;return(data??[])as TableMap[K][]}
